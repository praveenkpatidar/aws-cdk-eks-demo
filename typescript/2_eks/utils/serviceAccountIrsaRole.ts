 ...
 */
export function createServiceAccountIrsaRole(
    scope: Construct,
    ...
    const conditions = new cdk.CfnJson(scope, 'ConditionJson', {
        value: {
            ...
            [`${cluster.openIdConnectProvider.openIdConnectProviderIssuer}:sub`]: `system:serviceaccount:${namespace}:${serviceAccountName}`,
        },
    });
    ...
    for (const policyArn of awsManagedPolicies) {
        irsaRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName(policyArn));
    }
    // Create Service Account. If Namespace is not exists it will create one.
    if (createServiceAccount) {
        const namespaceServiceAccount = new eks.KubernetesManifest(scope, 'namespaceServiceAccount', {
            cluster,
            manifest: [
                {
                    apiVersion: 'v1',
